...
**Карта ремесла.** `Temari-Obsidian/` — хранилище Obsidian, собранное из кода игры. Вход — «00 Карта ремесла»: стадия числами, схема механик прямо в заметке и строка на механику; каталог (узоры, разметки, стежки, нити, размеры шара) лежит внутри заметок механик в `01 Механики/`. Состояние механики считается замыканием импортов от `src/pages.tsx` и `src/lab.ts`, а не пишется руками. Отдельной картинки больше нет: схема живёт в теле заметки. Руками не правится: `npx tsx scripts/build-craft-vault.mts --write`, CI проверяет `--check`. На чём стоят наши числа — [docs/assumptions.md](docs/assumptions.md).
